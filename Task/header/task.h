...
/*Function that creates a Task.
  ...
  Arguments:  'time_to_run' - time for execution in seconds from 1970
  ...
  Undefined B.: NULL ptr of task_op*/
...
/*Function that executes a task function.
  ...
  Arguments: 'task' - Task pointer. wont change except if user function does
  
  Return value: it will return RESCHEDULE or DON'T_RESCHEDULER according to the user 
...
/*Function that retrieves the value of the tasks' execution time in seconds since 1970.
...
/*Function that updates the time of a task to its next execution time.
  
  Arguments: 'task' - pointer to Task,
  
  Return value: The next execution time in seconds since 1970.
...
/*Function that checks if a task and another task are the same.
  
  Arguments: 'task' - pointer to Task,  UID
...
/*Function that compares tasks based on their current execution time.
